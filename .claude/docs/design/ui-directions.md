...
Search **is** built — it is row 1 of the top bar — and this document simply never recorded it. It is written down now under *The top bar*, from a screenshot rather than from the canvas, so treat the numbers there as observed and not as decided. **Five of the six questions standing here are closed by [`autofill.md`](autofill.md) (D63), built 31 Aug**: the focus state (border to `line strong` plus the crimson halo every other field takes), what it matches (item names, item **sizes** and term names — never notes, and a **word prefix** rather than a substring), whether it is debounced (it is not, on either the menu or the grid, and that becomes a question at a few hundred items rather than at twenty), the no-results screen (the item grid's empty state — Playfair italic over a meta line and one ghost control, never the run list's status disc, which would be the status ramp saying something about stock when this is about a query), and whether it reaches the term lists (it does, through the menu's `TERMS` group — but only to *offer* a term, never to fold its items into the results). **The field also gained the `×` this document has claimed it has since *Applied filters* was written**, and Escape now closes the menu first and clears the field second. **`Clear filters` still deliberately does not touch it** — and is doubly right now, since the menu's term rows put chips in that bar.
